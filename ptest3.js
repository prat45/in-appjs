function initTempoSurvey() {
  ...
  function toggleOtherCheckbox(checkId, boxId){
  ...
  function toggleOtherRadio(radioName, otherRadioId, boxId){
    var radios = r.querySelectorAll('input[name="' + radioName + '"]');
    var box = document.getElementById(boxId);
    if(!radios || !box) return;

    for(var j = 0; j < radios.length; j++){
      radios[j].addEventListener("change", function(){
        var otherRadio = document.getElementById(otherRadioId);
        box.style.display = (otherRadio && otherRadio.checked) ? "block" : "none";
      });
    }
  }
  ...
  function showPage(){
    ...
    if(g <= (TOTAL_QUESTIONS + 1)){
      o.style.width = ((g - 1) / TOTAL_QUESTIONS * 100) + "%";
      c.textContent = (g > 1 ? (g - 1) : 1) + " / " + TOTAL_QUESTIONS;
    } else {
      o.style.width = "100%";
      c.textContent = TOTAL_QUESTIONS + " / " + TOTAL_QUESTIONS;
    }

    d.style.display = (g === 1 || g === 7) ? "none" : "flex";
    i.style.display = g <= 2 ? "none" : "block";
    s.textContent = g === 6 ? "Kirim" : "Berikutnya";
  }
  ...
  function buildTrackingData(){
    return {
      ...
      "Attribute_3": formatWithOther(checkboxValues("q3"), textValue("q3Comment")),
      "Attribute_4": formatWithOther(radioValue("q4"), textValue("q4Comment")),
      "Attribute_5": radioValue("q5"),
